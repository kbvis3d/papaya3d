...
papaya.viewer.Viewer.prototype.load3dVolume = function () {
    ...
    this.initializeVolume(surface);
};


papaya.viewer.Viewer.prototype.initializeVolume = function (surface) {
    ...
    if (!surface.error) {
        ...
        if (this.volumeView === null) {
            this.lowerImageBot2 = this.volumeView = new papaya.viewer.Volume3D(this.volume, this.currentScreenVolume.colorTable, this, this.container.params);
        ...
        this.mainImage = this.volumeView;
...
papaya.viewer.Viewer.prototype.updatePosition = function (viewer, xLoc, yLoc, crosshairsOnly) {
    ...
    } else if (viewer.volumeView && this.insideScreenSlice(viewer.volumeView, xLoc, yLoc, viewer.volumeView.screenDim,
            viewer.volumeView.screenDim)) {
        viewer.volumeView.updateDynamic(originalX, originalY, (this.selectedSlice === this.mainImage) ? 1 : 3);
    }

    if (viewer.hasVolume()) {
        viewer.volumeView.updateVolume(viewer.currentCoord, this.draggingSliceDir);
    }
...
papaya.viewer.Viewer.prototype.updateCursorPosition = function (viewer, xLoc, yLoc) {
    ...
    if (this.container.display) {
        ...
        if (this.insideScreenSlice(viewer.axialSlice, xLoc, yLoc, viewer.volume.getXDim(), viewer.volume.getYDim())) {
            ...
            found = true;
        } else if (this.insideScreenSlice(viewer.volumeView, xLoc, yLoc)) {
            xLoc -= viewer.volumeView.screenOffsetX;
            yLoc -= viewer.volumeView.screenOffsetY;

            surfaceCoord = this.volumeView.pick(xLoc, yLoc);
...
papaya.viewer.Viewer.prototype.insideScreenSlice = function (screenSlice, xLoc, yLoc, xBound, yBound) {
    ...
    if (screenSlice === this.volumeView) {
...
papaya.viewer.Viewer.prototype.drawViewer = function (force, skipUpdate) {
    ...
    if (this.hasVolume() && (!papaya.utilities.PlatformUtils.smallScreen || force || (this.selectedSlice === this.volumeView))) {
...
papaya.viewer.Viewer.prototype.hasVolume = function () {
    return (this.container.hasSurface() && this.volumeView && this.volumeView.initialized);
};
...
papaya.viewer.Viewer.prototype.drawScreenSlice = function (slice) {
    ...
    if (slice === this.volumeView) {
        this.context.fillStyle = this.volumeView.getBackgroundColor();
        ...
        //if (this.container.preferences.showRuler === "Yes") {
        //    if (this.volumeView === this.mainImage) {
        ...
        //        this.context.fillText(this.volumeView.getRulerLength().toFixed(2), (slice.screenDim / 2) + textWidth - (offset / 2), papaya.viewer.Viewer.ORIENTATION_MARKER_SIZE + padding);
        //    }
        //}
    } else {
...
papaya.viewer.Viewer.prototype.drawOrientation = function () {
    ...
    if (this.mainImage === this.volumeView) {
        return;
    }
...
papaya.viewer.Viewer.prototype.getTransformParameters = function (image, height, lower, factor) {
    ...
    if (image === this.volumeView) {
        this.volumeView.resize(this.viewerDim / bigScale);
        return;
    }
...
papaya.viewer.Viewer.prototype.mouseDownEvent = function (me) {
    ...
    if ((me.target.nodeName === "IMG") || (me.target.nodeName === "CANVAS")) {
        if (me.handled !== true) {
            ...
            if (((me.button === 2) || this.isControlKeyDown || this.isLongTouch) && this.container.contextManager && (this.selectedSlice === this.mainImage) && (this.mainImage === this.volumeView)) {
                ...
                if (this.container.contextManager.prefersColorPicking && this.container.contextManager.prefersColorPicking()) {
                    pickedColor = this.volumeView.pickColor(this.contextMenuMousePositionX, this.contextMenuMousePositionY);
                ...
                this.isOpacityControl = this.isControlKeyDown;
                ...
                if (this.selectedSlice === this.volumeView) {
                    ...
                    this.volumeView.setStartDynamic(this.previousMousePosition.x, this.previousMousePosition.y);
                ...
                if (this.selectedSlice && (this.selectedSlice !== this.volumeView)) {
                ...
                } else if (this.selectedSlice && (this.selectedSlice === this.volumeView)) {
                    if (this.volumeView.findProximalRulerHandle(this.previousMousePosition.x - this.canvasRect.left,
...
papaya.viewer.Viewer.prototype.mouseUpEvent = function (me) {
    ...
    if ((me.target.nodeName === "IMG") || (me.target.nodeName === "CANVAS")) {
        if (me.handled !== true) {
            if (!this.isWindowControl && !this.isZoomMode && !this.isContextMode && (this.grabbedHandle === null) && (!this.volumeView || (this.volumeView.grabbedRulerPoint === -1))) {
    ...
    if (this.hasVolume()) {
        if (this.volumeView.grabbedRulerPoint === -1) {
            this.volumeView.updateCurrent();
        } else {
            this.volumeView.grabbedRulerPoint = -1;
        }
...
papaya.viewer.Viewer.prototype.findClickedSlice = function (viewer, xLoc, yLoc) {
    ...
    } else if (this.insideScreenSlice(viewer.volumeView, xLoc, yLoc, viewer.volume.getYDim(),
            viewer.volume.getZDim())) {
        this.selectedSlice = this.volumeView;
    } else {
        this.selectedSlice = null;
    }
};
...
papaya.viewer.Viewer.prototype.mouseMoveEvent = function (me) {
    ...
    if (this.isDragging) {
        if (this.grabbedHandle) {
            ...
            if (this.isOpacityControl)
                this.opacityChanged(this.previousMousePosition.x - currentMouseX, this.previousMousePosition.y - currentMouseY, true);
            else
            ...
            if (this.selectedSlice === this.volumeView) {
                this.volumeView.updateTranslateDynamic(papaya.utilities.PlatformUtils.getMousePositionX(me),
            ...
            if (this.selectedSlice === this.volumeView) {
                zoomFactorCurrent = ((this.previousMousePosition.y - currentMouseY) * 0.5) * this.volumeView.scaleFactor;
                this.volumeView.zoom += zoomFactorCurrent;
                this.previousMousePosition.x = currentMouseX;
                this.previousMousePosition.y = currentMouseY;
            } else {
            ...
            if (this.selectedSlice !== null) {
                if (this.selectedSlice === this.volumeView) {
                    if (this.volumeView.grabbedRulerPoint !== -1) {
                        this.volumeView.pickRuler(currentMouseX - this.canvasRect.left,
                            currentMouseY - this.canvasRect.top);
                        this.drawViewer(false, true);
                    } else {
                        this.volumeView.updateDynamic(papaya.utilities.PlatformUtils.getMousePositionX(me),
                            papaya.utilities.PlatformUtils.getMousePositionY(me), (this.selectedSlice === this.mainImage) ? 1 : 3);
                        this.drawViewer(false, true);
                        this.container.display.drawEmptyDisplay();
                    }
...
papaya.viewer.Viewer.prototype.windowLevelChanged = function (contrastChange, brightnessChange) {
    ...
    if (Math.abs(contrastChange) > Math.abs(brightnessChange)) {
        ...
        if (maxFinal <= minFinal) {
            ...
            maxFinal = this.currentScreenVolume.screenMin;
        }
    } else {
    ...
    this.volumeView.updateLut(minFinal, maxFinal, false);

    if (this.container.showImageButtons) {
        this.container.toolbar.updateImageMenuRange(this.getCurrentScreenVolIndex(), parseFloat(minFinal.toPrecision(4)),
            parseFloat(maxFinal.toPrecision(4)));
    }

    this.drawViewer(true);
};

papaya.viewer.Viewer.prototype.opacityChanged = function (contrastChange, brightnessChange) {
    var range, step, minFinal, maxFinal;

    range = this.opacityMax - this.opacityMin;
    step = range * 0.005;

    if (Math.abs(contrastChange) > Math.abs(brightnessChange)) {
        minFinal = this.opacityMin + (step * papaya.utilities.MathUtils.signum(contrastChange));
        maxFinal = this.opacityMax + (-1 * step * papaya.utilities.MathUtils.signum(contrastChange));

        if (maxFinal <= minFinal) {
            minFinal = this.opacityMin;
            maxFinal = this.opacityMin; // yes, min
        }
    } else {
        minFinal = this.opacityMin + (step * papaya.utilities.MathUtils.signum(brightnessChange));
        maxFinal = this.opacityMax + (step * papaya.utilities.MathUtils.signum(brightnessChange));
    }
    this.opacityMin = Math.max(0, Math.min(255, minFinal))
    this.opacityMax = Math.max(this.opacityMin, Math.min(255, maxFinal))

    this.volumeView.updateLut(this.opacityMin, this.opacityMax, true);
...
*/
    ...
    if (isSliceScroll) {
        ...
        if (scrollSign !== 0) {
            ...
            if (this.mainImage === this.volumeView) {
                this.volumeView.zoom += ((scrollSign * -5) * this.volumeView.scaleFactor);
                this.drawViewer(false, true);
            } else {
                if (this.mainImage.sliceDirection === papaya.viewer.ScreenSlice.DIRECTION_AXIAL) {
...
papaya.viewer.Viewer.prototype.isShowingVolume = function () {
    return (this.volumeView && this.volumeView.showSurfacePlanes);
};


papaya.viewer.Viewer.prototype.isShowingSurfaceCrosshairs = function () {
    return (this.volumeView && this.volumeView.showSurfaceCrosshairs);
};

papaya.viewer.Viewer.prototype.isShowingAxialPlane = function () {
    return (this.container.preferences.showAxialPlane === "Yes");
};
papaya.viewer.Viewer.prototype.isShowingCoronalPlane = function () {
    return (this.container.preferences.showCoronalPlane === "Yes");
};
papaya.viewer.Viewer.prototype.isShowingSagittalPlane = function () {
    return (this.container.preferences.showSagittalPlane === "Yes");
};
